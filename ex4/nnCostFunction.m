...
z2 = Theta1 * a1';
z2_sigmoid_gradient = sigmoidGradient(z2);
a2 = [bias sigmoid(z2)'];
...
for i = 1:m
  ...
  expected(y(i)) = 1;
  for k = 1:num_labels
    J = J + (-expected(k) .* log(h(i,k))) - ((1 - expected(k)) * log(1 - h(i,k)));
    delta3(k) = h(i,k) - expected(k);
  endfor
  delta2 = (Theta2(:,2:end)' * delta3) .* z2_sigmoid_gradient(:,i);
